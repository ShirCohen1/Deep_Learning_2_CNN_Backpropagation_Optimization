...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        input = torch.maximum(self.alpha*x, x)
        #split derivative by cases
        input[self.alpha * x > x] = self.alpha
        input[x >= self.alpha * x ] = 1
        dx = input * dout
        # ========================
...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        self.db += torch.sum(dout, dim=0) ### review formula 
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def backward(self, dout=1.0):
        ...
        dx = ((torch.div(e_x.T, e_x_sum).T - specific_ones) * dout) / N
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            mask = torch.bernoulli(torch.ones(x.shape) * (1-self.p)).int()
            out = x * mask
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        layers.extend([Linear(in_features, hidden_features[0]), activation_cls()])

        #add hidden layers
        if dropout > 0:
            layers.append(Dropout(dropout))
        for i in range(len(hidden_features)-1):
            layers.extend([Linear(hidden_features[i], hidden_features[i+1]), activation_cls()])
            if dropout > 0:
                layers.append(Dropout(dropout))
