    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            actual_num_epochs += 1
            train_loss_per_epoch, train_acc_per_epoch = self.train_epoch(dl_train, **kw)
            
            train_loss.extend(train_loss_per_epoch)
            train_acc.append(train_acc_per_epoch)

            test_loss_per_epoch, test_acc_per_epoch = self.test_epoch(dl_test, **kw)
            
            test_loss.extend(test_loss_per_epoch)
            test_acc.append(test_acc_per_epoch)
            # If no improvement to accuracy
            if test_acc_per_epoch <= best_acc:
              epochs_without_improvement += 1

            #if avg loss per epoch - averaged on batches did not improve
            # avg_loss = sum(test_loss_per_epoch) / len(test_loss_per_epoch)
            # if avg_loss >= best_loss :
            #   epochs_without_improvement += 1
              #If reached early stopping streak without improvement then end 
              if early_stopping and (epochs_without_improvement >= early_stopping):
                break
            
            else:
              best_acc =  test_acc_per_epoch 
              # best_loss = avg_loss
              #Restart count if acc improves
              epochs_without_improvement = 0
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # Forward pass
        #calling model that returns forward
